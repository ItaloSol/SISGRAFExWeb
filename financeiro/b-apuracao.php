          ...
          <div class="table-responsive text-nowrap">
            <table class="table table-bordered">
              ...
              <tbody>
                ...
                while ($Percorrer < $numero_clientes) {
                  if (number_format($Saldo_Correto[$Percorrer], 2, ',', '.') != number_format($Tabela_Clientes[$Percorrer]['credito'], 2, ',', '.')) {        ?>
                    <tr>
                      ...
                      <td>
                        <button class="btn rounded-pill btn-info" id="<?= $Tabela_Clientes[$Percorrer]['cod'] ?>" name="<?= $tipo_cliente ?>" onclick="ApurarCredito(this.id, this.name)">
                          <i class="bx bx-edit-alt me-1"></i> Corrigir
                        </button>
                      </td>
                      <td>
                    ...
                    <tr>
                      <td colspan="7">Apuração: CRÉDITO = <?= number_format($Valor_Notas_Totais[$Percorrer], 2, ',', '.') ?> DÉBITO = <?= number_format($Total_Faturamentos[$Percorrer], 2, ',', '.') ?> EM PRODUÇÃO = <?= number_format($Total_EmProducao[$Percorrer], 2, ',', '.') ?> TOTAL = <?= number_format($Saldo_Correto[$Percorrer], 2, ',', '.') ?> </td>
                    </tr>
                <?php $encontrados++;
                  }
                  $Percorrer++;
                ...
                <tr>
                  <td colspan="4" rowspan="4">TOTAL ENCONTRADOS: <?= $encontrados ?></td>
                </tr>
              </tbody>
            </table>
